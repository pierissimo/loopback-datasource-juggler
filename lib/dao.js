...
DataAccessObject.find = function find(query, options, cb) {
  ...
  var allCb = function(err, data) {
    if (!err && Array.isArray(data)) {
      handleNotifyAll(data, function (data) {
        async.map(data, function(item, next) {
    ...
    function handleNotifyAll(data, callback) {
      if (options.notify === false) return callback(data);
      ...
      Model.notifyObserversOf('loaded all', context, function(err) {
        if (err) return callback(err);
